...
const en = `Welcome to Orynx.

Agents need a runtime before they can execute work. You can still use Orynx as a lightweight project-management workspace while you install one.
...
When the runtime is connected, you can create Orynx Helper for a guided first run.`;

const zh = `欢迎来到 Orynx。

智能体需要先连上运行时才能执行工作。运行时还没准备好时,你也可以先把 Orynx 当作轻量项目管理工具体验起来。
...
运行时连上后,你就可以创建 Orynx Helper,开始一次有智能体参与的上手引导。`;

const ko = `Orynx에 오신 것을 환영합니다.

agent가 작업을 실행하려면 먼저 runtime이 필요합니다. runtime을 설치하는 동안에도 Orynx를 가벼운 프로젝트 관리 워크스페이스로 먼저 사용할 수 있습니다.
...
runtime이 연결되면 Orynx Helper를 만들어 안내를 받으며 첫 실행을 시작할 수 있습니다.`;

const ja = `Orynx へようこそ。

agent が作業を実行するには、まず runtime が必要です。runtime をインストールしている間も、Orynx を軽量なプロジェクト管理ワークスペースとして先に使うことができます。
...
runtime が接続されたら、Orynx Helper を作成して、案内付きの最初の実行を始められます。`;
