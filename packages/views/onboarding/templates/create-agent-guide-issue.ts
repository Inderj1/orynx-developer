...
const HELPER_AGENT_NAME = "Orynx Helper";

/**
 * Skip path, issue 2/2: "Create your first Orynx Agent".
 *
...
export const CREATE_AGENT_GUIDE_ISSUE_TITLE = {
  en: "Step 2 — Create your first Orynx Agent",
  zh: "第 2 步 —— 创建你的第一个 Orynx Agent",
  ko: "2단계 — 첫 Orynx Agent 만들기",
  ja: "ステップ2 — 最初の Orynx Agent を作成する",
} as const;
...
function enBody(installRuntimeMention: string): string {
  return `Once your runtime is online (see ${installRuntimeMention}), build your first agent — Orynx Helper. The prompt below is pre-written; just copy.
...
Now create an issue (or reassign an existing one) → set assignee = Orynx Helper → set status to **todo**. The runtime picks the task up within a few seconds and starts working. Watch progress in the issue's task panel.
...
function zhBody(installRuntimeMention: string): string {
  return `等运行时上线（见 ${installRuntimeMention}）之后，把第一个 agent —— Orynx Helper —— 建出来。下面的提示词已经写好，直接复制即可。
...
接着创建一个 issue（或把已有 issue 重新分派）→ 把 assignee 设成 Orynx Helper → 状态切到 **todo**。运行时会在几秒内接走任务并开始工作。在 issue 的任务面板里看进度。
...
function koBody(installRuntimeMention: string): string {
  return `runtime이 online 상태가 되면(${installRuntimeMention} 참고), 첫 agent인 Orynx Helper를 만드세요. 아래 prompt는 미리 작성되어 있으니 그대로 복사하면 됩니다.
...
이제 issue를 만들거나 기존 issue를 다시 배정한 뒤 assignee를 Orynx Helper로 설정하고 status를 **todo**로 바꾸세요. runtime이 몇 초 안에 작업을 가져가 실행을 시작합니다. 진행 상황은 issue의 task panel에서 볼 수 있습니다.
...
function jaBody(installRuntimeMention: string): string {
  return `runtime が online になったら(${installRuntimeMention} を参照)、最初の agent である Orynx Helper を作りましょう。下の prompt はあらかじめ書いてあるので、そのままコピーするだけです。
...
次に issue を作る(または既存の issue を割り当て直す)→ assignee を Orynx Helper にする → status を **todo** にします。runtime が数秒以内にタスクを受け取って作業を始めます。進捗は issue の task panel で確認できます。
