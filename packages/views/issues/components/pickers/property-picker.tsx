...
export function PropertyPicker({
  ...
  // Base UI's Trigger assumes a native <button> by default, which is correct for
  // almost every triggerRender (a native <button>, or a <Button>/<PillButton>
  // component that renders one). Only force nativeButton=false when the trigger
  // is an intrinsic non-button element (e.g. the context menu's positioning
  // <span>), so Base UI applies role/keyboard button behavior instead of
  // warning. Components are left at the default since they typically render a
  // <button>; leaving it undefined otherwise falls back to Base UI's native path.
  const triggerNativeButton =
    isValidElement(triggerRender) &&
