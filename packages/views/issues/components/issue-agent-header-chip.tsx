...
function ActiveChip({ issueId, running, queued }: ActiveChipProps) {
  ...
  // Earliest running start → a live, ticking elapsed timer so the chip visibly
  // advances every second. This is the clearest "it's actually progressing"
  // signal in the fixed header spot, next to the existing border-beam.
  const runningStart = anyRunning ? earliestStart(running) : null;
  const elapsed = useLiveElapsed(runningStart);
    ...
    <div className="flex items-center gap-1">
      <Popover>
        ...
        <PopoverTrigger
          ...
          render={
            <button
              ...
              className={cn(
                "flex h-7 max-w-[14rem] items-center gap-1.5 rounded-md px-1.5 text-muted-foreground outline-none transition-colors hover:bg-accent/60 focus-visible:ring-2 focus-visible:ring-ring",
        ...
        >
          ...
          {anyRunning && elapsed ? (
            <span className="shrink-0 text-[11px] tabular-nums text-info/70">
              {elapsed}
            </span>
          ) : null}
        </PopoverTrigger>
...
// Earliest start among the running tasks. Falls back to dispatched_at when a
// task reports no explicit run start yet, so the timer begins the moment work
// is in flight rather than snapping in late.
function earliestStart(tasks: AgentTask[]): string | null {
  let earliest: string | null = null;
  for (const task of tasks) {
    const s = task.started_at ?? task.dispatched_at ?? null;
    if (!s) continue;
    if (!earliest || new Date(s).getTime() < new Date(earliest).getTime()) {
      earliest = s;
    }
  }
  return earliest;
...
// Ticks once a second while a run is active. Returns null when there's no start
// time (queued-only), so the caller renders nothing rather than a stuck "0s".
function useLiveElapsed(startIso: string | null): string | null {
