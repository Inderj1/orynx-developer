    ...
    <div className="relative flex h-full min-w-0 flex-1 flex-col">
        ...
        <BreadcrumbHeader
          ...
          actions={
            ...
            {/* Live goal-progress: how far the sub-issues (stages) have gotten.
                Answers "where are we / how much is done?" at a glance, in the
                fixed header. Driven by the same live childProgress cache, so it
                fills as agents finish stages — no refresh needed. */}
            {childIssues.length > 0 && (() => {
              const goalTotal = childIssues.length;
              const goalDone = childIssues.filter((c) => c.status === "done").length;
              // Agent-completed work parks in `in_review` awaiting a human, so
              // count it as visible progress (a lighter segment) — otherwise the
              // bar reads 0 even after agents finished several stages.
              const goalReview = childIssues.filter(
                (c) => c.status === "in_review",
              ).length;
              const donePct = Math.round((goalDone / goalTotal) * 100);
              const reviewPct = Math.round((goalReview / goalTotal) * 100);
              const goalComplete = goalDone >= goalTotal;
              const toDo = goalTotal - goalDone - goalReview;
              return (
                <div
                  className="flex h-7 items-center gap-1.5 rounded-md border border-border bg-muted/40 px-2"
                  ...
                  <div className="flex h-1.5 w-16 overflow-hidden rounded-full bg-border">
                    <div
                      className={cn(
                        ...
                        goalComplete ? "bg-info" : "bg-info",
                      )}
                      style={{ width: `${donePct}%` }}
                    />
                    {/* Lighter segment = finished by an agent, awaiting review. */}
                    <div
