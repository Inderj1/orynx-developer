...
describe("WelcomeAfterOnboarding", () => {
  ...
  describe("runtime path", () => {
    it("creates a Helper agent then opens a blocking modal with starter cards", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/welcome to Orynx/i)).toBeInTheDocument();
      });

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.runtime_id).toBe("rt-1");
      expect(agentArgs.name).toBe("Orynx Helper");
      expect(agentArgs.instructions).toContain("Orynx Helper");

      // 3 starter card titles come from HELPER_STARTER_PROMPTS (TS const,
      // EN under the test's en locale).
      expect(
        screen.getByText("Introduce Orynx to me"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Walk me through the core features"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Show me what Orynx can do for me — as slides"),
      ).toBeInTheDocument();
    });

    it("reuses an existing Orynx Helper agent instead of creating duplicates", async () => {
      mockListAgents.mockResolvedValueOnce([
        {
          id: "agent-existing",
          name: "Orynx Helper",
    ...
    it("selecting cards then clicking Assign creates one issue per pick and navigates to the first", async () => {
      ...
      // Toggle two cards.
      fireEvent.click(screen.getByText("Introduce Orynx to me"));
      fireEvent.click(
        screen.getByText("Show me what Orynx can do for me — as slides"),
      );
      ...
      expect(titles).toEqual([
        "Introduce Orynx to me",
        "Show me what Orynx can do for me — as slides",
      ]);
    ...
    it("uses Korean persisted Helper and starter issue artifacts under ko locale", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("Orynx를 간단히 소개해 주세요"),
        ).toBeInTheDocument(),
      );

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.description).toContain("Orynx 사용 어시스턴트");
      expect(agentArgs.instructions).toContain(
        "당신은 이 Orynx 워크스페이스에 내장된 AI 어시스턴트",
      );

      fireEvent.click(screen.getByText("Orynx를 간단히 소개해 주세요"));
      fireEvent.click(
      ...
      expect(issueArgs.title).toBe("Orynx를 간단히 소개해 주세요");
      expect(issueArgs.description).toContain(
        "Orynx를 1-2문단으로 간단히 소개해 주세요",
      );
    });
    ...
    it("uses Japanese persisted Helper and starter issue artifacts under ja locale", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("Orynx を簡単に紹介してください"),
        ).toBeInTheDocument(),
      );

      expect(mockCreateAgent).toHaveBeenCalledTimes(1);
      const [agentArgs] = mockCreateAgent.mock.calls[0]!;
      expect(agentArgs.description).toContain("Orynx の使い方アシスタント");
      expect(agentArgs.instructions).toContain(
        "あなたは Orynx Helper、この Orynx ワークスペースに組み込まれた AI アシスタント",
      );

      fireEvent.click(screen.getByText("Orynx を簡単に紹介してください"));
      fireEvent.click(
      ...
      await waitFor(() => expect(mockCreateIssue).toHaveBeenCalledTimes(1));
      const [issueArgs] = mockCreateIssue.mock.calls[0]!;
      expect(issueArgs.title).toBe("Orynx を簡単に紹介してください");
      expect(issueArgs.description).toContain(
        "Orynx を1〜2段落で簡単に紹介してください",
      );
    });
  });
  ...
  describe("skip path", () => {
    it("provisions install-runtime → agent-guide → follow-up comment, then opens the celebration Modal", async () => {
      ...
      expect(secondCall![0].title).toBe(
        "Step 2 — Create your first Orynx Agent",
      );
    ...
    it("silently dismisses without showing the Modal when provisioning fails", async () => {
      ...
      expect(screen.queryByText(/Welcome to Orynx/i)).not.toBeInTheDocument();
    });
    ...
    it("uses Korean persisted skip-path issue and comment artifacts under ko locale", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Orynx에 오신 것을 환영합니다/i)).toBeInTheDocument();
      });
    ...
    it("uses Japanese persisted skip-path issue and comment artifacts under ja locale", async () => {
      ...
      expect(installCall![0].description).toContain("Orynx へようこそ。");
      expect(guideCall![0].title).toBe(
        "ステップ2 — 最初の Orynx Agent を作成する",
      );
