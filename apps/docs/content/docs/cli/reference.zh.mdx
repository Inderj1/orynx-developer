---
title: CLI Reference
description: Complete command reference for the Orynx CLI and agent daemon.
---

The `multica` CLI connects your local machine to Orynx. It handles authentication, workspace management, issue tracking, and runs the agent daemon that executes AI tasks locally.
...
The daemon is the local agent runtime. It detects available AI CLIs on your machine, registers them with the Orynx server, and executes tasks when agents are assigned work.
...
When connecting to a self-hosted Orynx instance, point the CLI to your server before logging in:
