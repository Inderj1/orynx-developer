...
The Orynx server scans for due triggers every **30 seconds** — **the actual fire time can lag by up to 30 seconds**, not down to the second. If the server is restarted across a fire time, it catches up missed triggers on startup (nothing is lost, but they fire right away).
...
on the autopilot detail page; Orynx generates a unique URL of the shape:

```
https://<your-multica-host>/api/webhooks/autopilots/awt_…
```

POST any JSON to that URL. Orynx first stores a durable delivery and
...
…or any JSON object/array. Orynx normalizes it into an internal envelope:
...
When you don't provide an `event` field, Orynx infers it from common
...
comma-separated **actions** list. Orynx allows a webhook if **any**
...
Orynx derives the `event` name and `action` from the inbound request
...
**2. Headers.** When no body envelope is present, Orynx reads the
...
present, Orynx falls back to top-level body string fields in this
...
**Action candidates, in full.** Once the event is determined, Orynx
...
Orynx acknowledges an active, valid delivery only after both its delivery and
...
custom self-hosted reverse proxies. Orynx deliberately does not derive
