...
description: Common issues when self-hosting Orynx — symptoms, causes, how to diagnose, how to fix.
---
...
| `smtp RCPT TO <addr>: 550 5.7.1 Unable to relay` | The relay's receive connector doesn't allow your subnet to relay to external recipients (most common for anonymous internal relays talking to outside domains) | Either restrict invites to internal recipients, or add the Orynx host's subnet to the Exchange "Anonymous Users → Relay" permission list |
...
- `Unable to relay` → either restrict to internal recipients or grant the Orynx host's IP relay permission on the Exchange receive connector
