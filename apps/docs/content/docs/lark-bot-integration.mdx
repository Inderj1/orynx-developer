...
description: Bind a Orynx agent to a Lark (飞书) Bot, then talk to it from a Lark DM or group — @-mention it, chat naturally, or type /issue to file a Orynx issue without leaving Lark.
---

import { Callout } from "fumadocs-ui/components/callout";

Bind any [agent](/agents) to a Lark (飞书) Bot and your team can work with it from inside Lark — DM the Bot, @-mention it in a group, or type `/issue` to file a [Orynx issue](/issues) without opening the app. The agent's replies stream back into the chat as a live card that updates while it works.

Each Bot is bound **one-to-one** to a single Orynx agent. Binding a second agent creates a second Bot; one agent never has two Bots.
...
| **DM the Bot** | A workspace member messages the Bot directly in Lark. Each conversation becomes a Orynx [chat](/chat) session with the agent; the agent answers in-thread. |
...
| **`/issue` command** | Typing `/issue <title>` (optionally with a body) creates a new Orynx issue in the workspace, attributed to you. |
...
4. When the scan completes the dialog closes and the agent shows **Connected to Lark**. Your own Lark identity is bound to your Orynx account automatically, so you can start chatting with the Bot right away.
...
The first time you message the Bot, it replies with a card asking you to **bind your Lark identity**. Tap the link, sign in to Orynx, and your Lark account is linked to your Orynx membership. This is what lets the agent act as you — for example, `/issue` files the issue under your name.
...
- **File an issue** — send `/issue Fix the login redirect` and Orynx creates that issue in the workspace, assigned the way any new issue would be. Add more lines after the title for a description.
...
- [Chat](/chat) — what a Bot conversation maps to inside Orynx
