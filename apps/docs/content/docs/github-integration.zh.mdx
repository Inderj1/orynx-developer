...
仓库范围由你授予 GitHub App 的权限决定（全部仓库或指定子集）。在工作区连接这个 installation 本身就是一次订阅——Orynx 里不需要再单独逐仓库勾选。
...
PR **关闭但没合并**——只更新 PR 卡片的状态为 `Closed`，issue 状态不变。"关闭但不合并"语义因团队而异，Orynx 不替用户做决定。
...
- **App 没安装的仓库里的 PR**——没 App，Orynx 收不到 webhook
- **手动把 PR 关联到 issue**——暂时没有这个 UI。如果你们的约定把编号放到 Orynx 不扫的地方，请改放到 PR 标题或正文里

## 断开连接

**Settings → GitHub** 里没有 installation 列表——现有 installation 直接到 GitHub 上管理：

- **从 GitHub 卸载** —— 个人在 `https://github.com/settings/installations`、组织在 `https://github.com/organizations/<org>/settings/installations` 卸载 Orynx App。Orynx 收到 `installation.deleted` webhook 后立刻删行；任何已打开的 Settings tab 实时更新，不用刷新
- **Orynx 这边的断开是 admin only** —— GitHub tab 上的 Disconnect 控件对非 admin 不显示；主开关关掉时 Disconnect 仍然可用，方便 admin 一键关闭功能后再单独清理已连接的 installation
...
- GitHub App 申请的是 PR 和 Metadata 的 **只读** 权限。Orynx 从不向 GitHub 推 commit、评论或 status check

## Self-Host 配置

如果你在 Orynx Cloud 上，集成已经配好——跳过本节。
...
| 字段 | 值 |
|---|---|
| **GitHub App name** | 任何能辨识的名字，例如 `Orynx` 或 `Orynx (staging)` |
| **Homepage URL** | 你的 Orynx 前端，例如 `https://multica.example.com` |
...
<Callout type="warning">
**Webhook secret ≠ Client secret。** App 设置页里两个字段紧挨着。**Webhook secret** 用于签 `pull_request` payload，这才是 Orynx 需要的那个；**Client secret** 是 OAuth 用的，和本集成无关。混淆这两个会得到「每条 webhook 都 `401 invalid signature`」的诡异症状。
</Callout>
...
- `/api/webhooks/github` 直接返回 **`503 github webhooks not configured`**——Orynx 在 secret 没配置时拒绝处理事件，不会出现「没 secret 也接受 webhook」的安全坑

`GITHUB_APP_ID` 和 `GITHUB_APP_PRIVATE_KEY` **可选**。配上之后，setup 回调可以用 App JWT 鉴权调用 GitHub `/app/installations/{id}`，安装完成那一刻就拿到真实的组织名/用户名。不配的话，连接卡片会先显示 `已连接到 unknown`，等 GitHub 的 `installation.created` webhook 到达（通常几秒内），Orynx 会刷新 row 并通过 realtime 推送让 Settings 页面无需手动刷新即可更新。私钥从 App 设置页 **Private keys → Generate a private key** 生成，把整段 PEM（含 `-----BEGIN/END RSA PRIVATE KEY-----` 两行）粘到 env 里，保留换行。
...
- **不读 CI / check 状态**——只镜像 PR 本身，构建状态、reviewer 评论、reviewer 列表都没接进 Orynx
