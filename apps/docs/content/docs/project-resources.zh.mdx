...
这个形状是有意的 —— 跟 Orynx 里 agent provider 的设计完全一致：一个 `type` 判别字段 + 一个有类型的 payload。schema 保持稳定，未来加"Notion 页面"、"Google Doc"、"上传文件"、"外部 URL"都是小型的、增量的改动。
...
跟 `github_repo` 相比的取舍是有意的：只有绑定的那台守护进程会使用这个本地目录执行任务，而且 **同一个目录上的任务串行执行**，不再并行。换来的是：你的现有 checkout、当前分支、未提交的脏改动一切照旧 —— Orynx 不会重新 clone。
...
黑名单故意做得激进 —— 选你自己的 home 目录就意味着 Orynx 的运行时文件会出现在你账号的根下，这不会是任何人想要的结果。请选一个子目录（一般就是你具体的项目 checkout）。
...
4. Orynx 自己的运行时产物（`output/`、`logs/`、`.gc_meta.json`）放在 **你目录之外** 的一个独立 envRoot 里。
...
- **不会自动开 PR。** 任务结束后，改动就停在它实际做出来的分支上 —— Orynx 不会 push、不会开 PR。需要 push、需要 PR 的话请自己来。
