...
description: Get started with Orynx Cloud — no setup required.
---

The fastest way to get started with Orynx — no setup required.
...
```
Fetch https://github.com/multica-ai/multica/blob/main/CLI_INSTALL.md and follow the instructions to install Orynx CLI, log in, and start the daemon on this machine.
```
...
Open your workspace in the Orynx web app. Navigate to **Settings → Runtimes** — you should see your machine listed as an active **Runtime**.

> **What is a Runtime?** A Runtime is a compute environment that can execute agent tasks. It can be your local machine (via the daemon) or a cloud instance. Each runtime reports which agent CLIs are available, so Orynx knows where to route work.
