...
description: Deploy Orynx on your own infrastructure.
---

## Architecture Overview

Orynx has three components:
...
This installs the CLI, checks out the latest self-host assets, pulls the official Orynx images from GHCR, and configures everything for localhost. Then open http://localhost:3000 and pick a login method: configure `RESEND_API_KEY` in `.env` for email-based codes (recommended), or leave Resend unset and copy the generated code from backend logs. See [Step 2 — Log In](#step-2--log-in) for details.
...
3. Leave the `pg_cron` extension itself installed unless you are sure no other workload depends on it. The bundled `pgvector/pgvector:pg17` image does **not** ship `pg_cron`, so nothing in Orynx's default install needs it; uninstalling `pg_cron` from a custom image that other workloads still use is a separate decision.
...
## Switching to Orynx Cloud

If you've been self-hosting and want to switch your CLI to [Orynx Cloud](https://multica.ai):
...
Orynx uses email-based magic link authentication via [Resend](https://resend.com).
...
Orynx requires PostgreSQL 17 with the pgvector extension.
