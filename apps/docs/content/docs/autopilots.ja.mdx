...
Orynx サーバーは**30 秒**ごとに期限が来たトリガーをスキャンします — **実際の発火時刻は最大 30 秒まで遅れる可能性があり**、秒単位で正確ではありません。発火時刻のあたりでサーバーが再起動された場合、起動時に逃したトリガーを追いつきます（何も失われませんが、すぐに発火します）。
...
オートパイロットはインバウンドの HTTP webhook でも発火できます。オートパイロット詳細ページで **Webhook** トリガーを追加すると、Orynx は次のような形の一意の URL を生成します。

```
https://<your-multica-host>/api/webhooks/autopilots/awt_…
```

その URL に任意の JSON を POST してください。Orynx はまず配信を永続化し、冪等な実行を同期的に受け付け、`status = accepted | skipped` と `run_id` を含む `200 OK` を返します。その後データベース-backed worker が accepted の実行を再開してエージェントをディスパッチします。受付とディスパッチの間にサーバーが再起動しても、キュー済みの配信は失われません。
...
…または任意の JSON オブジェクト / 配列を送ることもできます。Orynx はこれを内部エンベロープに正規化します。
...
`event` フィールドを指定しない場合、Orynx は一般的なヘッダーと本文フィールドからこれを推論します（`X-GitHub-Event` + 本文 `action`、`X-Gitlab-Event`、`X-Event-Type`、本文の `event`/`type`/`action`）。どれも一致しない場合、イベントは `webhook.received` になります。
...
各行は 1 つのルールです。**イベント名（event name）**と、任意でカンマ区切りの **action** リストで構成されます。Orynx は**いずれか 1 つ**の行でも一致すれば webhook を許可します。セクションを空のままにすると、すべてを受け付けます（フィルタリング以前の動作）。
...
Orynx は次の順序でインバウンドリクエストから `event` 名と `action` を導き出します — **最初に一致したものが優先されます**。
...
**2. ヘッダー（Headers）。** 本文エンベロープがない場合、Orynx は次のよく知られたプロバイダーヘッダーを読みます。
...
**3. 本文フォールバック（Body fallback）。** 本文エンベロープも既知のヘッダーもない場合、Orynx は次の順序で最上位の本文文字列フィールドにフォールバックします: `event` → `type` → `action`。
...
**action 候補、全リスト。** イベントが決定されると、Orynx は以下のすべての値を可能な action 一致対象として考慮します。
...
サーバーに `MULTICA_PUBLIC_URL` が設定されている場合（例: `https://multica.example.com`）、トリガー応答に絶対パスの `webhook_url` が含まれ、UI にはすぐにコピーできる URL が表示されます。設定しない場合、UI はクライアントの API origin から URL を構成します — デスクトップと同一オリジンの Web には問題ありませんが、カスタムのセルフホストリバースプロキシには適しません。Orynx は、誤って構成されたリバースプロキシが攻撃者の制御するホストを指す webhook URL をサーバーに発行させて欺くことができないよう、`Host` / `X-Forwarded-Host` ヘッダーから公開ホストを導出しないよう意図的に設計されています。
