---
title: 聊天集成（channels）
description: Orynx 如何把智能体接入聊天平台——一个统一的 channel 引擎，加上针对飞书（Lark）和 Slack 的各平台适配器——涵盖入站流水线、会话与授权。
---

import { Callout } from "fumadocs-ui/components/callout";
import { Mermaid } from "@/components/mermaid";

**channel** 把一个 Orynx [智能体](/agents)接入聊天平台，团队就能在他们日常沟通的地方直接使用它。目前有两个 channel——[Lark（飞书）](/lark-bot-integration) 和 [Slack](/slack-bot-integration)——两者都跑在**同一个引擎**上：一个平台无关的内核，加上一层很薄的各平台适配器。新增一个平台是「实现适配器」，而不是「重建流水线」。
...
4. **身份 + 授权** —— 把发送者的平台用户 id 解析成一个 Orynx 用户（即[账号绑定](#账号绑定)），然后再次校验 workspace 成员身份。未绑定的发送者会收到一条「绑定你的账号」提示；非成员会被丢弃。
...
- **账号绑定（认证）** —— 发送者的平台用户 id 必须关联到一个 Orynx 用户。某人第一次给 Bot 发消息时，会拿到一个一次性链接，把自己的身份绑定到**他自己的** Orynx 账号；在那之前不会有任何智能体运行。
- **Workspace 成员身份（授权）** —— 绑定后的 Orynx 用户必须是该安装所属 workspace 的成员，每条消息都会重新校验。非成员会被静默丢弃。
...
<Callout type="info">
**Slack —— 自带应用。** workspace 管理员创建一个 Slack app，把它安装到自己的 Slack workspace，再把它的 bot token + app-level token 粘贴进 Orynx。每个智能体都有自己的 Slack app，所以多个智能体可以在同一个 Slack workspace 里各自拥有一个独立的 Bot。manifest 和分步设置见 [Slack Bot 接入](/slack-bot-integration)。
</Callout>
...
在 Orynx Cloud 上两者都已配置好。完整参考见[环境变量](/environment-variables)。
