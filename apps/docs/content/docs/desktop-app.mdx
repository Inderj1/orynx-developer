...
Grab the installer for your platform from the [Orynx downloads page](https://multica.ai/download):
...
<Callout type="info">
**Desktop defaults to Orynx Cloud, but can be pointed at a self-hosted instance with a local config file.** There is still no in-app "connect to self-host" picker. Desktop reads `~/.multica/desktop.json` before the renderer starts; if the file is missing, it uses the Cloud defaults.
