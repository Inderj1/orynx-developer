...
4. **Identity + authorization** — resolve the sender's platform user id to a Orynx user (the [account binding](#authorization)), then re-check workspace membership. Unbound senders get a "link your account" prompt; non-members are dropped.
...
- **Account binding (authentication)** — the sender's platform user id must be linked to a Orynx user. The first time someone messages the bot they get a one-time link to bind their identity to **their own** Orynx account; until then no agent runs.
- **Workspace membership (authorization)** — the bound Orynx user must be a member of the installation's workspace, re-checked on every message. Non-members are silently dropped.
...
On Orynx Cloud both are already configured. See [Environment variables](/environment-variables) for the full reference.
