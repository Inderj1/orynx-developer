...
description: Agents don't run on Orynx's servers — they run on your own machines.
---

import { Callout } from "fumadocs-ui/components/callout";
import { Mermaid } from "@/components/mermaid";

In Orynx, [agents](/agents) do **not** run on our servers — they run on your own machines, driven by a small program called the **daemon** that invokes the [AI coding tools](/providers) installed locally. The Orynx server only coordinates: it stores [issues](/issues), queues [tasks](/tasks), and dispatches them to the right **runtime** (runtime = daemon × one AI coding tool).

This structure is the biggest difference between Orynx and Linear / Jira: **your API keys, toolchain, and code directories stay on your machine** — the Orynx server never sees any of them. That means "my agent isn't working" is almost always a local problem — the daemon isn't running, an AI tool isn't installed, a key has expired. Check locally first; see [Troubleshooting](/troubleshooting) for a guide.

## Starting the daemon

The daemon is part of the Orynx CLI. Once you've installed the [Orynx CLI](/cli), run on your own machine:
...
A runtime is not a server and not a container — it's the combination of "**daemon × one AI coding tool**". For example: you start the daemon on a MacBook with both Claude Code and Codex installed, and you're a member of two workspaces. Orynx then registers 4 runtimes:
...
- The **Runtimes** page in the Orynx UI lists these rows

## Custom runtime profiles

Built-in provider detection covers the common tools, but teams can also define
**custom runtime profiles** for AI CLIs that speak one of Orynx's supported
...
- Install the custom command on at least one machine running a Orynx daemon,
...
- Confirm the tool speaks one of the base protocol families shown in the UI.
  Choosing a family tells Orynx how to communicate with the process; it does
...
The command is argv-oriented, not a shell string. Orynx stores an executable
...
<Callout type="info">
**Cloud runtimes are coming**, currently in a waitlist phase. Once available, you'll be able to execute agent tasks directly on Orynx Cloud without running a local daemon. Sign up with your email on the [download page](https://multica.ai/download) to get notified.
</Callout>

## When a runtime is marked offline

Orynx uses heartbeats to decide whether a runtime is online. Three key numbers:
...
**Tasks running on a missing runtime are marked as failed** (failure reason `runtime_offline`). For retryable sources (issues, chat), Orynx automatically requeues them; Autopilot-triggered tasks are not retried automatically. See [Tasks → Which failures retry automatically](/tasks#which-failures-retry-automatically-which-dont).
</Callout>

## How many tasks can run in parallel

Orynx enforces concurrency limits at two layers:
...
3. Open the **Runtimes** page in the Orynx UI — confirm your runtime shows "online"
