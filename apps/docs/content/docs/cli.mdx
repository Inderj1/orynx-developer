...
description: One-page overview of every top-level Orynx CLI command. For full usage, run `multica <command> --help`.
---

import { Callout } from "fumadocs-ui/components/callout";

The Orynx CLI mirrors almost everything the Web UI can do (create [issues](/issues), assign [agents](/agents), start the [daemon](/daemon-runtimes), and more). This page lists every top-level command with a one-line description. For the full set of flags and examples, run `multica <command> --help`.
...
| `multica setup cloud` | One-shot setup for Orynx Cloud (login + install daemon) |
