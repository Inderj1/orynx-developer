---
title: CLI 命令速查
description: Orynx CLI 的所有顶级命令一页概览。完整用法查 `multica <命令> --help`。
---

import { Callout } from "fumadocs-ui/components/callout";

Orynx CLI 把 Web UI 能做的事几乎全部搬到了命令行上（创建 [issue](/issues)、分配 [智能体](/agents)、启动 [守护进程](/daemon-runtimes) 等等）。这一页把所有顶级命令列出来，每条配一句用途。完整 flag 和示例用 `multica <命令> --help` 查。
...
| `multica setup cloud` | Orynx Cloud 一键初始化（登录 + 装 daemon） |
