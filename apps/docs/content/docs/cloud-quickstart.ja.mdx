...
このページは Orynx Cloud を最初から最後まで案内します — **サインアップ → [CLI](/cli) のインストール → [デーモン](/daemon-runtimes)の起動 → [エージェント](/agents)の作成 → 最初の[タスク](/tasks)の割り当て**。約 5 分かかります。
...
1. CLI が Orynx Cloud に接続するよう構成します
...
4. 完了すると結果を Orynx に報告します（自動リトライが作動するかどうかに応じて、ステータスが `completed` または `failed` になります）
