...
오토파일럿은 [에이전트](/agents)가 **스케줄에 따라 자동으로 작업을 시작**하게 합니다 — cron 표현식과 타임존을 설정하면, 여러분이 아무것도 트리거하지 않아도 Orynx가 알아서 [`task`](/tasks)를 디스패치합니다. 정기 점검, 반복 보고서, 야간 정리 작업 등 "상시 지시(standing order)" 형태의 작업에 잘 맞습니다. 나머지 세 가지 트리거 경로([할당](/assigning-issues), [@-멘션](/mentioning-agents), [채팅](/chat) — 모두 여러분이 직접 시작하는 방식)와 비교했을 때, 오토파일럿의 핵심 차이는 **시간 기반**이라는 점입니다.
...
Orynx 서버는 **30초**마다 만료된 트리거를 스캔합니다 — **실제 발화 시각은 최대 30초까지 지연될 수 있으며**, 초 단위로 정확하지는 않습니다. 발화 시각 즈음에 서버가 재시작되면, 시작 시 놓친 트리거를 따라잡습니다(아무것도 유실되지 않지만 즉시 발화됩니다).
...
오토파일럿은 인바운드 HTTP webhook으로도 발화할 수 있습니다. 오토파일럿 상세 페이지에서 **Webhook** 트리거를 추가하면, Orynx가 다음과 같은 형태의 고유 URL을 생성합니다.

```
https://<your-multica-host>/api/webhooks/autopilots/awt_…
```

그 URL로 아무 JSON이나 POST하세요. Orynx는 먼저 전달을 영구 저장하고 멱등 실행을 동기적으로 승인한 뒤, `status = accepted | skipped`와 `run_id`가 포함된 `200 OK`를 반환합니다. 이후 데이터베이스-backed worker가 accepted 실행을 재개하고 에이전트를 디스패치합니다. 수신과 디스패치 사이에 서버가 재시작되어도 큐에 저장된 전달은 유실되지 않습니다.
...
…또는 임의의 JSON 객체/배열을 보낼 수도 있습니다. Orynx는 이를 내부 봉투로 정규화합니다.
...
`event` 필드를 제공하지 않으면, Orynx는 일반적인 헤더와 본문 필드로부터 이를 추론합니다(`X-GitHub-Event` + 본문 `action`, `X-Gitlab-Event`, `X-Event-Type`, 본문의 `event`/`type`/`action`). 어느 것도 일치하지 않으면 이벤트는 `webhook.received`가 됩니다.
...
각 행은 하나의 규칙입니다. **이벤트 이름(event name)**과 선택적으로 쉼표로 구분한 **action** 목록으로 구성됩니다. Orynx는 **어느 한** 행이라도 일치하면 webhook을 허용합니다. 섹션을 비워 두면 모든 것을 수락합니다(필터링 이전 동작).
...
Orynx는 다음 순서로 인바운드 요청에서 `event` 이름과 `action`을 도출하며 — **첫 번째로 일치하는 것이 우선합니다**.
...
**2. 헤더(Headers).** 본문 봉투가 없을 때, Orynx는 다음의 잘 알려진 제공자 헤더를 읽습니다.
...
**3. 본문 폴백(Body fallback).** 본문 봉투도 알려진 헤더도 없으면, Orynx는 다음 순서로 최상위 본문 문자열 필드로 폴백합니다: `event` → `type` → `action`.
...
**action 후보, 전체 목록.** 이벤트가 결정되면, Orynx는 아래의 모든 값을 가능한 action 일치 대상으로 고려합니다.
