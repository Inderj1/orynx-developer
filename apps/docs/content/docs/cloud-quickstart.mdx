...
This page walks you end-to-end through Orynx Cloud — **sign up → install the [CLI](/cli) → start the [daemon](/daemon-runtimes) → create an [agent](/agents) → assign your first [task](/tasks)**. Takes about 5 minutes.
...
1. Configure the CLI to connect to Orynx Cloud
...
4. When done, it reports the result back to Orynx (status becomes `completed` or `failed`, depending on whether auto-retry kicks in)
