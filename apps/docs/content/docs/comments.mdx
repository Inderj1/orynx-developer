...
To link another issue, choose it from the comment mention picker. Orynx stores issue links as an explicit `[MUL-123](mention://issue/<uuid>)` mention link. Issue links are cross-references only: they do not notify people and they do not trigger agents.
...
But Orynx's defining trait hasn't entered the picture yet: **treating agents as first-class members of a workspace**. That's what we turn to next.
