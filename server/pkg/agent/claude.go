...
// filterInheritedEnv strips env vars that must never reach a spawned agent
// process: Claude Code's internal per-session markers AND host/daemon secrets
// (DB credentials, JWT secret, server API keys, etc.). Agent content is
// untrusted (prompt injection via issue/comment text), so an agent must not be
// handed the credentials of the process that launched it. Agent-provider keys
// (ANTHROPIC_*, OPENAI_*, XAI_API_KEY, GITHUB_TOKEN, ...) are intentionally
// preserved so the agent can still authenticate to its own backend.
func filterInheritedEnv(base []string) []string {
	out := make([]string, 0, len(base))
	for _, entry := range base {
		key, _, _ := strings.Cut(entry, "=")
		if isFilteredChildEnvKey(key) || isSensitiveHostEnvKey(key) {
			continue
		}
...
func mergeEnv(base []string, extra map[string]string) []string {
	filtered := filterInheritedEnv(base)
	env := make([]string, 0, len(filtered)+len(extra))
	env = append(env, filtered...)
	// Task-scoped vars (e.g. the agent's mat_ token) are added after filtering
	// so they are always delivered, and so a task can legitimately provide its
	// own DATABASE_URL etc. for the app it is building without inheriting the
	// daemon's.
	for k, v := range extra {
		env = append(env, k+"="+v)
	}
...
// isSensitiveHostEnvKey reports whether an inherited env var is a host/daemon
// secret that must not be exposed to an agent subprocess. This is a targeted
// denylist rather than a strict allowlist so it never accidentally strips the
// arbitrary vars agent CLIs legitimately need (locale, proxies, provider keys);
// the trade-off is that a NEW server secret must be added here (a stricter
// allowlist is the ideal follow-up). Agent-provider credentials (ANTHROPIC_*,
// OPENAI_*, XAI_API_KEY, GITHUB_TOKEN, ...) do NOT match and stay available.
func isSensitiveHostEnvKey(key string) bool {
	switch key {
	case "JWT_SECRET",
		"DATABASE_URL", "REDIS_URL",
		"POSTGRES_PASSWORD", "POSTGRES_USER", "POSTGRES_DB",
		"RESEND_API_KEY", "COMPOSIO_API_KEY", "MULTICA_LLM_API_KEY",
		"GOOGLE_CLIENT_SECRET", "GOOGLE_CLIENT_ID",
		"SMTP_PASSWORD", "SMTP_USERNAME",
		"MULTICA_SLACK_SECRET_KEY", "MULTICA_LARK_SECRET_KEY",
		"MULTICA_DEV_VERIFICATION_CODE",
		"CLOUDFRONT_KEY_PAIR_ID", "CLOUDFRONT_PRIVATE_KEY",
		"STRIPE_SECRET_KEY", "STRIPE_WEBHOOK_SECRET":
		return true
	}
	// Defensive pattern for anything explicitly named a secret / password. The
	// provider keys above (…_API_KEY, …_TOKEN) intentionally do not match.
	upper := strings.ToUpper(key)
	return strings.Contains(upper, "_SECRET") || strings.HasSuffix(upper, "PASSWORD")
}
