...
func discoverACPModels(ctx context.Context, executablePath string, p acpDiscoveryProvider) ([]Model, error) {
	...
	hideAgentWindow(cmd)
	// Strip Claude markers + host/daemon secrets before handing the env to the
	// agent subprocess (same guard as buildEnv); task-scoped extraEnv is added
	// after so it is always delivered.
	childEnv := append(filterInheritedEnv(os.Environ()), p.extraEnv...)
	cmd.Env = childEnv
