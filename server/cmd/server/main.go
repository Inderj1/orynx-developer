...
func main() {
	...
	if os.Getenv("JWT_SECRET") == "" {
		// Fail closed in production: the built-in default secret is public in
		// the repo, so booting with it would let anyone forge a token for any
		// user. Local/dev deploys keep the convenient default.
		if strings.EqualFold(strings.TrimSpace(os.Getenv("APP_ENV")), "production") {
			slog.Error("JWT_SECRET must be set when APP_ENV=production — refusing to start with the built-in insecure default")
			os.Exit(1)
		}
		slog.Warn("JWT_SECRET is not set — using the insecure built-in default. Never do this outside local development.")
	}
