...
func (h *Handler) DeleteIssue(w http.ResponseWriter, r *http.Request) {
	...
	// Cancel any in-flight agent tasks first. This is a service call with its
	// own side effects (broadcasts, task-row updates), so it stays outside the
	// delete transaction — but if it fails we abort before deleting anything,
	// so we never orphan a cancelled-task/undeleted-issue mismatch.
	if err := h.TaskService.CancelTasksForIssue(r.Context(), issue.ID); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to cancel issue tasks")
		return
	}

	// Collect all attachment URLs (issue-level + comment-level) before the row
	// is gone, so the S3 cleanup below has the list to delete.
	...
	// Atomic delete: fail linked autopilot runs and delete the issue in one
	// transaction — all-or-nothing. Previously these ran as separate unchecked
	// statements, so FailAutopilotRunsByIssue could succeed while the delete
	// failed, leaving failed-autopilot rows pointing at a still-present issue.
	...
	// Fail linked autopilot runs before delete (ON DELETE SET NULL clears issue_id).
	...
	// Post-commit: external cleanup + notifications. The row is already gone,
	// so a failure here can't corrupt DB state.
